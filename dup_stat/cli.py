...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        metavar="TIMESTAMP",
        help=(
            "Добавить результаты этого запуска к уже сохранённым — "
            "укажите timestamp из имени прежнего файла результата "
            "(например 20260823_071202, как в "
            "'dup_stat_results_20260823_071202.json'). Прежние записи и "
            "новые объединяются, группы пересобираются заново и "
            "сортируются вместе. Прежние файлы результата НЕ изменяются: "
            "объединённый результат сохраняется в НОВЫЙ файл, а новый "
            "timestamp просто дописывается к переданному через '_' — "
            "например 'dup_stat_results_20260823_071202_20260824_090000.json'. "
            "При повторном добавлении укажите полный timestamp из имени "
            "этого нового файла — цепочка будет расти с каждым запуском. "
            "Требует --save-dir (там же должны лежать прежние файлы "
            "результата, включая .json)."
        ),
    )
    return parser
...
def main(argv=None) -> int:
    ...
    if args.save_dir is not None:
        ...
        # previous_timestamp=None в обычном режиме — используется просто
        # свежий timestamp. В режиме --append-timestamp он дописывается
        # к переданному через "_" (см. ResultPersistence.save_all),
        # поэтому имя файла со временем превращается в цепочку из всех
        # timestamp'ов, через которые прошли эти данные.
        save_result = persistence.save_all(groups, args.save_dir, previous_timestamp=args.append_timestamp)
